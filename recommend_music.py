from db import users_collection, music_collection
...
def load_music():
    return list(music_collection.find())


def personality_match_score(music, user_personality):
    total_weight = sum(user_personality.values())

    if total_weight == 0:
        return 0

    personality_scores = music.get("personalityScores", {})

    weighted_sum = 0

    for ptype in PERSONALITY_TYPES:
        music_score = float(personality_scores.get(ptype, 0) or 0)
        user_weight = user_personality.get(ptype, 0)

        weighted_sum += music_score * user_weight

    return weighted_sum / total_weight


def mood_match_score(music, todays_moods):
    if not todays_moods:
        return 0

    mood_scores = music.get("moodScores", {})
...
def score_music(music_list, user_personality, todays_moods):
    scored = []
    ...
    for music in music_list:
        ...
        scored.append((final_score, music))
...
def print_top(scored, n=10):
    ...
    for score, music in scored[:n]:
        print(f"{score:6.2f}  {music.get('title', 'Unknown Title')}")
...
if __name__ == "__main__":
    ...
    print()
    print(f"Music Recommendations for {username}")
    print()

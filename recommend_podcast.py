from db import users_collection, podcasts_collection
...
def load_podcasts():
    return list(podcasts_collection.find())


def personality_match_score(podcast, user_personality):
    total_weight = sum(user_personality.values())

    if total_weight == 0:
        return 0

    personality_scores = podcast.get("personalityScores", {})

    weighted_sum = 0

    for ptype in PERSONALITY_TYPES:
        podcast_score = float(personality_scores.get(ptype, 0) or 0)
        user_weight = user_personality.get(ptype, 0)

        weighted_sum += podcast_score * user_weight

    return weighted_sum / total_weight


def mood_match_score(podcast, todays_moods):
    if not todays_moods:
        return 0

    mood_scores = podcast.get("moodScores", {})
...
def score_podcasts(podcasts, user_personality, todays_moods):
    scored = []
    ...
    for podcast in podcasts:
        ...
        scored.append((final_score, podcast))
...
def print_top(scored, n=10):
    ...
    for score, podcast in scored[:n]:
        print(f"{score:6.2f}  {podcast.get('title', 'Unknown Title')}")
...
if __name__ == "__main__":
    ...
    print()
    print(f"Podcast Recommendations for {username}")
    print()

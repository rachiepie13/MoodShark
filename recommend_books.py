...
def personality_match_score(book, user_personality):
    total_weight = sum(user_personality.values())

    if total_weight == 0:
        return 0

    personality_scores = book.get("personalityScores", {})

    weighted_sum = 0

    for ptype in PERSONALITY_TYPES:
        book_score = float(personality_scores.get(ptype, 0) or 0)
        user_weight = user_personality.get(ptype, 0)

        weighted_sum += book_score * user_weight

    return weighted_sum / total_weight


def mood_match_score(book, todays_moods):
    if not todays_moods:
        return 0

    mood_scores = book.get("moodScores", {})
...
def score_books(books, user_personality, todays_moods):
    scored = []
...
def print_top(scored, n=10):
    ...
    for score, book in scored[:n]:
        print(f"{score:6.2f}  {book.get('title', 'Unknown Title')}")
...
if __name__ == "__main__":
    ...
    print()
    print(f"Book Recommendations for {username}")
    print()
